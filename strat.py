...
class CommodityTermStructureAlgorithm(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.Schedule.On(self.DateRules.MonthStart(
            "SPY"), self.TimeRules.AfterMarketOpen("SPY", 30), self.Rebalance)
    ...
    def OnData(self, slice):
        ...
        for chain in slice.FutureChains:
            ...
            if chain.Value.Symbol.Value not in self.chains:
                self.chains[chain.Value.Symbol.Value] = [
                    i for i in chain.Value]
    ...
    def Rebalance(self):
        ...
        for symbol, chain in self.chains.items():
            ...
            price_near = near_contract.LastPrice if near_contract.LastPrice > 0 else 0.5 * \
                float(near_contract.AskPrice+near_contract.BidPrice)
            price_distant = distant_contract.LastPrice if distant_contract.LastPrice > 0 else 0.5 * \
                float(distant_contract.AskPrice+distant_contract.BidPrice)

            if distant_contract.Expiry == near_contract.Expiry:
                self.Debug(
                    "ERROR: Near and distant contracts have the same expiry!" + str(near_contract))
                return
            expire_range = 365 / \
                (distant_contract.Expiry - near_contract.Expiry).days
            roll_returns[symbol] = (
                np.log(float(price_near)) - np.log(float(price_distant)))*expire_range
            positive_roll_returns = {
                symbol: returns for symbol, returns in roll_returns.items() if returns > 0}
            negative_roll_returns = {
                symbol: returns for symbol, returns in roll_returns.items() if returns < 0}

        backwardation = sorted(
            positive_roll_returns, key=lambda x: positive_roll_returns[x], reverse=True)[:quintile]
        contango = sorted(negative_roll_returns,
                          key=lambda x: negative_roll_returns[x])[:quintile]
